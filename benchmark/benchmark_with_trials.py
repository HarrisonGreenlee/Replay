...
def main():
    ...
    for scenario_name, config in SCENARIOS.items():
        ...
        # Explicitly wipe any existing CSV before writing (belt + suspenders).
        if os.path.exists(csv_file):
            try:
                os.remove(csv_file)
                print(f"[INFO] Removed existing CSV: {csv_file}")
            except Exception as e:
                print(f"[WARNING] Could not remove existing CSV {csv_file}: {e}")

        with open(csv_file, mode="w", newline="") as f:
            writer = csv.writer(f)
            # Header now includes trial number and network seed
            header = [
                "trial",
                "population",
                "steps",
                "num_edges",
                "network_seed"
            ] + [f"{ctype}_runtime" for ctype in compute_types]
            writer.writerow(header)

            # Track persistent failures per compute type (skip remaining work for that type)
            ...
            for pop_size in POPULATION_SIZES:
                if all(skip_map.values()):
                    ...
                    # Still emit one row per trial with NaNs so downstream parsing doesn't break
                    for trial in range(1, NUM_TRIALS + 1):
                        writer.writerow([trial, pop_size, STEPS, math.nan, math.nan] + [math.nan] * len(compute_types))
                    f.flush()
                    os.fsync(f.fileno())
                    continue
                ...
                for trial in range(1, NUM_TRIALS + 1):
                    # Different network generation seed for each trial
                    network_seed = deterministic_seed(scenario_name, pop_size, trial)

                    # Unique graph filename per (scenario, pop_size, trial)
                    graph_file = f"{scenario_name}_{pop_size}_trial{trial}.txt"
                    ...
                    print(f"[INFO] Generating graph for {scenario_name}, population={pop_size}, trial={trial} (seed={network_seed})...")
                    gen_time = run_subprocess(gen_cmd)
                    if gen_time is None:
                        print(f"[WARNING] Generation of {graph_file} failed or timed out.")
                        writer.writerow([trial, pop_size, STEPS, math.nan, network_seed] + [math.nan] * len(compute_types))
                        f.flush()
                        os.fsync(f.fileno())
                    ...
                    try:
                        ...
                        print(f"[INFO] Read {num_edges:,} edges from {graph_file}.")
                    except Exception as e:
                        print(f"[ERROR] Could not read number of edges from {graph_file}: {e}")
                        writer.writerow([trial, pop_size, STEPS, math.nan, network_seed] + [math.nan] * len(compute_types))
                        f.flush()
                        os.fsync(f.fileno())
                        # Clean up
                    ...
                    for ctype, ctype_args in compute_types.items():
                        ...
                        print(f"[INFO] Running {ctype.upper()} simulation for {scenario_name}, population={pop_size}, trial={trial}...")
                        sim_time = run_subprocess(sim_cmd)

                        if sim_time is None:
                            print(f"[WARNING] {ctype.upper()} simulation for population={pop_size}, trial={trial} failed.")
                            skip_map[ctype] = True  # Mark as failed for subsequent runs
                            run_times[ctype] = math.nan
                        else:
                            run_times[ctype] = sim_time
                            print(f"[INFO] {ctype.upper()} simulation completed in {sim_time:.2f} seconds.")

                    row = [trial, pop_size, STEPS, num_edges, network_seed] + [run_times[ctype] for ctype in compute_types]
                    writer.writerow(row)
                    f.flush()
                    os.fsync(f.fileno())
